...
from XMLWidget import XMLAttrTag, _XMLAttrTag
from XMLWidget import XMLListValue, _XMLListValue
...
class XMLScanner(Scanner):
	...
	def __scanXMLAttrTag(self, dsrc, exp, row):
		"""
		解释一个带属性的 XML 标签
		"""
		attrs = {}
		for attrName, attrValue in exp.attrs.iteritems():
			attrs[attrName] = self.__scanValue(dsrc, attrValue, row)
		tagName = self.__scanValue(dsrc, exp.keyExp, row)
		return _XMLAttrTag(tagName, **attrs)

	def __scanXMLListValue(self, dsrc, exp, row):
		"""
		解释一个 XML 标签组
		"""
		tagName = exp.tagName
		...
		return _XMLListValue(tagName, values)
	...
	def __scanValue(self, dsrc, eExp, row):
		...
		if vtype is dict or vtype is OrderDict:
			...
			value = eExp(dsrc, row)
		elif isinstance(eExp, XMLAttrTag):
			value = self.__scanXMLAttrTag(dsrc, eExp, row)
		elif isinstance(eExp, XMLListValue):
			value = self.__scanXMLListValue(dsrc, eExp, row)
		else:
			value = eExp
		return value
	...
	def __scanMainKey(self, dsrc, kExp, row):
		...
		if isinstance(kExp, ex_base):
			key = kExp(dsrc, row)
		elif isinstance(kExp, XMLAttrTag):
