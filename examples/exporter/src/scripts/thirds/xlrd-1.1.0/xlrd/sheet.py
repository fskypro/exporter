...
class Sheet(BaseObject):
    ...
    def put_cell_ragged(self, rowx, colx, ctype, value, data, xf_index):
        if ctype is None:
    ...
    def put_cell_unragged(self, rowx, colx, ctype, value, data, xf_index):
        if ctype is None:
            # we have a number, so look up the cell type
            ctype = self._xf_index_to_xl_type_map[xf_index]
        # assert 0 <= colx < self.utter_max_cols
        ...
        try:
            ...
            self._cell_texts[rowx][colx] = data
            if self.formatting_info:
            ...
            if nr <= self.nrows:
                ...
                if nextra > 0:
                    ...
                    self._cell_texts[rowx].extend([UNICODE_LITERAL('')] * nextra)
            else:
                scta = self._cell_types.append
                scva = self._cell_values.append
                scxa = self._cell_xf_indexes.append
                ...
                for _unused in xrange(self.nrows, nr):
                    ...
                    self._cell_texts.append([UNICODE_LITERAL('')] * nc)
                    if fmt_info:
                        scxa(bf * nc)
                self.nrows = nr
    ...
    def read(self, bk):
        ...
        while 1:
            ...
            if rc == XL_NUMBER:
                ...
                self_put_cell(rowx, colx, None, d, data, xf_index)
            elif rc == XL_LABELSST:
                rowx, colx, xf_index, sstindex = local_unpack('<HHHi', data)
                # print "LABELSST", rowx, colx, sstindex, bk._sharedstrings[sstindex]
                self_put_cell(rowx, colx, XL_CELL_TEXT, bk._sharedstrings[sstindex], data, xf_index)
                if do_sst_rich_text:
                ...
                self_put_cell(rowx, colx, XL_CELL_TEXT, strg, data, xf_index)
            elif rc == XL_RSTRING:
                rowx, colx, xf_index = local_unpack('<HHH', data[0:6])
                if bv < BIFF_FIRST_UNICODE:
                ...
                for colx in xrange(mulrk_first, mulrk_last+1):
                    ...
                    pos += 6
                    self_put_cell(mulrk_row, colx, None, d, data, xf_index)
            elif rc == XL_ROW:
                ...
                if result_str[6:8] == b"\xFF\xFF":
                    ...
                    if first_byte == 0:
                        ...
                        self_put_cell(rowx, colx, XL_CELL_BOOLEAN, value, data, xf_index)
                    elif first_byte == 2:
                        # Error in cell
                        value = BYTES_ORD(result_str[2])
                        self_put_cell(rowx, colx, XL_CELL_ERROR, value, data, xf_index)
                    elif first_byte == 3:
                        # empty ... i.e. empty (zero-length) string, NOT an empty cell.
                        self_put_cell(rowx, colx, XL_CELL_TEXT, "", data, xf_index)
                    else:
                ...
                self_put_cell(rowx, colx, cellty, value, data, xf_index)
            elif rc == XL_COLINFO:
                if not fmt_info: continue
                c = Colinfo()
                ...
                self_put_cell(rowx, colx, XL_CELL_BLANK, '', data, xf_index)
                ...
                for colx in xrange(mul_first, mul_last + 1):
                    self_put_cell(rowx, colx, XL_CELL_BLANK, '', data, result[pos])
                    pos += 1
            ...
            #### all of the following are for BIFF <= 4W
                ...
                if rc == XL_FORMAT or rc == XL_FORMAT2:
                    ...
                    self_put_cell(rowx, colx, None, d, data, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx))
                elif rc == XL_INTEGER:
                    rowx, colx, cell_attr, d = local_unpack('<HH3sH', data)
                    self_put_cell(rowx, colx, None, float(d), data, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx))
                elif rc == XL_LABEL_B2:
                    rowx, colx, cell_attr = local_unpack('<HH3s', data[0:7])
                    strg = unpack_string(data, 7, bk.encoding or bk.derive_encoding(), lenlen=1)
                    self_put_cell(rowx, colx, XL_CELL_TEXT, strg, data, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx))
                    ...
                    self_put_cell(rowx, colx, cellty, value, data, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx))
                elif rc == XL_BLANK_B2:
                    if not fmt_info: continue
                    rowx, colx, cell_attr = local_unpack('<HH3s', data[:7])
                    self_put_cell(rowx, colx, XL_CELL_BLANK, '', data, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx))
                elif rc == XL_EFONT:
